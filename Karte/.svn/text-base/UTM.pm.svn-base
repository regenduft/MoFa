...
@EXPORT_OK
    = qw(ConvertDatum ConvToTM ConvFromTM %GRIDZN DegreesToGKK GKKToDegrees UTMToDegrees DegreesToUTM);
...
%GRIDDEF = (
    BWI =>
        [ 'BWI', 400000, 0, 'KM', [ 0, 1e6, 0, 1e6 ], [ 0, 10, -66, -55 ] ],
    CMP => [
        'CMP', 200000, 300000, 'KM',
        [ 64e3,  368e3, 0,     580e3 ],
        [ 36.95, 42.17, -9.66, -6.08 ], 'Lisboa'
    ],
    IcG => [
        'IcG', 500000, 500000, 'KM',
        [ 0,    1e6, 0,   1e6 ],
        [ 63.5, 65,  -30, -10 ],
        'Hjorsey 1955'
    ],
    LV03 => [
        'LV03', 200000, 600000, 'KM',
        [ 0,  1e6, 0, 1e6 ],
        [ 45, 48,  6, 11 ], 'CH-1903'
    ],
    RDG => [
        'RDG', 155000, 463000, 'KM',
        [ 0,    29e4,  29e4, 63e4 ],
        [ 50.3, 53.45, 3,    7.45 ],
...
sub DegreesToUTM {
    ...
    if ( $lat >= -80 && $lat <= 84 ) {
        ...
        my (@cs)
            = ConvToTM( $lat, $long, $UTMlat0, $long0, $UTMk0, $datum )
            ;    #XXXsub
...
sub ConvToTM {
    ...
    $t = $t * $t;
    my $c = cos($phi);
    my $A = ( $lambda - $lambda0 ) * $c;
    $c = $et2 * $c * $c;
    my $x = sprintf "%.0f",
        (
        $k0 * $n * (
            $A + ( 1 - $t + $c ) * $A * $A * $A / 6
                + ( 5 - 18 * $t + $t * $t + 72 * $c - 58 * $et2 ) * $A * $A
                * $A * $A * $A / 120
        )
        );
    my $y = sprintf "%.0f",
...
sub ConvFromTM {
    ...
    my $mu = $m / (
        $a * (
            1 - $es / 4 - 3 * $es * $es / 64 - 0.01953125 * $es * $es * $es
        )
    );
    my $phi1
        = ( $mu + ( 1.5 * $e1 - 27 * $e1 * $e1 * $e1 / 32 ) * sin( $mu + $mu )
            + ( 1.3125 - 1.71875 * $e1 * $e1 ) * $e1 * $e1 * sin( 4 * $mu )
            + 1.572916666667 * $e1 * $e1 * $e1 * sin( 6 * $mu ) + +2.142578125
            * $e1 * $e1 * $e1 * $e1 * sin( 8 * $mu ) );
    my $c1 = cos($phi1);
    $c1 = $et2 * $c1 * $c1;
    my $t1 = tan($phi1);
    $t1 = $t1 * $t1;
    ...
    my $lat = (
        $phi1 - $n1 * tan($phi1) / $r1 * (
            $d * $d / 2
                - ( 5 + 3 * $t1 + 10 * $c1 - 4 * $c1 * $c1 - 9 * $et2 ) * $d
                * $d * $d * $d / 24 + (
                61 + 90 * $t1 + 298 * $c1 + 45 * $t1 * $t1 - 252 * $et2 - 3
                    * $c1 * $c1
                ) * $d * $d * $d * $d * $d * $d / 720
        )
    ...
    my $long = (
        $lambda0 + (
            $d - ( 1 + ( $t1 + $t1 ) + $c1 ) * $d * $d * $d / 6 + (
                5 - ( $c1 + $c1 ) + 28 * $t1 - 3 * $c1 * $c1 + 8 * $et2 + 24
                    * $t1 * $t1
                ) * $d * $d * $d * $d * $d / 120
            ) / cos($phi1)
    ) * 57.29577951308232087684;
...
sub TMAux {
    ...
    $a * ( ( 1 - $es / 4 - 0.046875 * $es2 - 0.01953125 * $es3 ) * $phi
            ...
            sin( 4 * $phi ) - 0.011393229166667 * $es3 * sin( 6 * $phi ) );
}
...
sub ConvToUPS {
    ...
    my $e      = sqrt($es);
    my $ee     = $e * sin($phi);
    my $t      = tan( 0.78539816339744830961 - $phi / 2 )
        / pow( ( 1 - $ee ) / ( 1 + $ee ), ( $e / 2 ) );
    my $rho = 2 * $a * $UPSk0 * $t /
...
sub ConvFromUPS {
    ...
    $y = $y - 2e6;
    my $rho = sqrt( $x * $x + $y * $y );
    my $t = $rho * sqrt( pow( 1 + $e, 1 + $e ) * pow( 1 - $e, 1 - $e ) )
        / ( 2 * $a * $UPSk0 );
...
%GDATUM = (
    ...
    "Potsdam" =>
        [ "Bessel 1841", 587, 16, 393, 102, "GPStrans has: 606 23 413" ],
);
