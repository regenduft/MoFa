    ...
    return (
        join(
            ...
            start_html(
                ...
                -title    => 'Ergebnis der Standortbestimmung',
                -onload   => 'load()',
                -onunload => 'GUnload()'
            ),
            ...
            div({   -id    => 'input_links',
                ...
                table(
                    ...
                    Tr( td('Alle Treff- punkte'),
                        td( scrolling_list(
                                {   -id       => '1id',
                                    -name     => '1id',
                                    -style    => 'width:18em;height:17em;',
                                    -onchange =>
                                        'change_map0([\'1id\'], [set_geodata] )',
                                    -values => [
                                        -1,
                                        (   sort {
                                                $meetingpts{$a}
...
sub display_inputs {
        my $prefix = shift();
    my $desc   = shift;
      return  div({   -id    => $prefix . 'map',
                -style =>
                    'border-width:1px;border-style:solid; width:19em; background:#F0F0F0; padding:0.5em;'
            },
            table(
                Tr( td('X_Koordinate'),
                    td( input(
...
sub display_map {
    ...
    return (
        div({   -id    => 'map' . $prefix,
                -style =>
                    'float:left;margin-left:0.5em;border-width:1px;border-style:solid;width: 35em; height: 32em;'
            },
            br()
        ),
