...
I<Rückgabewert:> String mit Get-Link zu der angebene Seite (mit Session-Id)
...
I<Rückgabewert:> String mit Post-Link zu der angebene Seite (mit Session-Id)
...
I<Rückgabewert:> C<string> mit zurück-Link
...
I<Rückgabewert:> falls angemeldet: C<string> mit Link um sich abzumelden;
...
I<Rückgabewert:> C<string> mit WML-Card: komplettes Login-Formular
...
I<Rückgabewert:> C<string> mit WML-Card:
...
I<Parameter:> Vorgaben für Startpunkte (Referenz auf Liste von C<Mofa::Model::Address>-Objekten),
Vorgaben für Endpunkte (Referenz auf Liste von C<Mofa::Model::Address>-Objekten),
URL an die Formular gesendet werden soll (C<string>)

I<Rückgabewert:> C<string> mit WML-Card: Suchformular mit Start, Ziel, Zeit
...
I<Rückgabewert:> C<string> mit WML-Card: Eingabeformular für neue MFG
...
I<Rückgabewert:> C<string> mit WML-Card: Infos/Impressum 
...
I<Rückgabewert:> C<string> mit WML-Card: Enthält Link der
...
I<Rückgabewert:> C<string> mit WML-Card: Startseite nach dem Einloggen.
...
sub card_personal_menu {
    return (
        card(
			...
			zu werden. Es verfällt jedoch beim Abmelden und muss gelöscht und nach dem
			nächstem Login neu angelegt werden.'
                )
            ),
            p(  { -align => 'center' },
...
I<Parameter:> Rückgabewert des SMS-Servicecenter (C<string>)
...
sub confirm_register {
    sendwml(
        card(
            ...
            p(  small(
                    'Ihre Telefonnumer wurde bestätigt.',
                    'Sie können den Mobile Hitchhiker nun in vollem Umfang nutzen'
                )
            ),
            p(  small(
...
Parameter für dieses Bild (X, Y, Radius, Höhe, Breite) als C<int>
...
Liste von C<Mofa::Address>-Objekten die zur Auswahl zur Verfügung stehen. 

I<Rückgabewert:> C<string> mit einer WML-Card mit einem Formular zur
...
sub display_search_result(@) {
    ...
    sendwml_nocache(
        card(
            ...
            p(  small(
                    'Sie erhalten eine WAP-Push-SMS sobald Sie jemand mitnehmen würde.',
                    br(),
                    'Nach der Registrierung erhalten Sie sofort einen Link per SMS, ',
                    'den sie aufrufen müssen um einmalig die Telefonnummer zu bestaetigen.',
                    'Danach benachrichtigen wir die Anbieter der MFGs über ihre Anfrage.',
                    br(),
...
I<Parameter>: Überschrift (C<string>), Nachrichten (C<(string,..)>).
...
sub _msg {
    return p( small( join( br(), @_ ), ) ), back_link(),
        ...
        (   (   defined( $session->param('login') )
            ...
            ? anchor( get_link('p_index.pl'), 'Persönliche Startseite' )
                . br()
            : ' '
        ),
